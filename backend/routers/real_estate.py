...
from models.user import User, UserProfile  # Import User first to ensure it's available for relationships
...
@router.get("/properties/{property_id}/title-deed")
async def generate_title_deed(property_id: int, db: Session = Depends(get_db)):
    """
    Generate Tanzanian Title Deed PDF for a property
    
    Returns the official title deed document in PDF format
    """
    try:
        ...
        if property.owner_id:
            # Try to get owner via relationship first, then fallback to direct query
            try:
                if hasattr(property, 'owner') and property.owner:
                    owner = property.owner
                else:
                    owner = db.query(UserProfile).filter(UserProfile.id == property.owner_id).first()
                
                if owner:
                    owner_data = {
                        'first_name': getattr(owner, 'first_name', ''),
                        'last_name': getattr(owner, 'last_name', ''),
                        'national_id_number': getattr(owner, 'national_id_number', None),
                        'address': getattr(owner, 'address', None) or getattr(owner, 'business_address', None),
                        'phone': getattr(owner, 'phone', None) or getattr(owner, 'business_phone', None),
                        'email': getattr(owner, 'email', None) or getattr(owner, 'business_email', None),
                    }
            except Exception:
                # If owner lookup fails, continue without owner data
                pass
        ...
        property_dict = {
            ...
            'lot_use': property.lot_use,
            'size': property.size,
            'area': property.area,
            'boundary_coordinates': property.boundary_coordinates,
            'boundary_points': property.boundary_points,
            'boundary_distance': property.boundary_distance,
            'national_id_number': property.national_id_number,
        ...
        pdf_bytes, filename = document_generator.generate_tanzanian_title_deed(
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate title deed: {str(e)}"
        )

@router.get("/properties/{property_id}/title-deed/base64")
async def generate_title_deed_base64(property_id: int, db: Session = Depends(get_db)):
    """
    Generate Tanzanian Title Deed PDF and return as base64 encoded string
    
    Useful for frontend to display or download
    """
    try:
        # Get property from database
        property = db.query(Property).filter(Property.id == property_id).first()
        ...
        return {
            "filename": filename,
            "document_type": "tanzanian_title_deed",
            "format": "pdf",
            "code": pdf_base64,
            "description": f"Official Tanzanian Title Deed for property {property.title}",
            "property_id": property.id,
            "lot_number": property.lot_number or property.id
        }
