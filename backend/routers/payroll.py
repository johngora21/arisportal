...
from models.payroll import Staff, Branch, Department, Role, PayrollRecord, PayrollCalculation, PayrollPayment, EmploymentStatus, EmploymentType, MaritalStatus, Gender
...
SHARED_BILLPAY_NAMBA = os.getenv('CLICKPESA_BILLPAY_NAMBA', '3864')  # ClickPesa merchant number

@router.post("/payroll/generate-payment")
async def generate_payroll_payment(
    ...
    """
    Generate BillPay control number for payroll payment.
    Calculates total amount including:
    - Sum of all net salaries
    - ClickPesa fees per payout (estimated)
    - 1% platform fee per payout
    - 1% settlement fees on total
    """
    try:
        # Get all payroll records for the period
        query = db.query(PayrollRecord).filter(PayrollRecord.payroll_period == payroll_period)
        ...
        payroll_records = query.join(Staff, PayrollRecord.staff_id == Staff.id).all()
        
        if not payroll_records:
            raise HTTPException(status_code=404, detail=f"No payroll records found for period {payroll_period}")
        
        # Check if payment already exists
        existing_payment = db.query(PayrollPayment).filter(
            PayrollPayment.payroll_period == payroll_period,
            PayrollPayment.branch_id == branch_id
        ).first()
        
        if existing_payment and existing_payment.status == "paid":
            raise HTTPException(status_code=400, detail="Payroll payment already completed for this period")
        
        # Calculate totals
        total_net_salary = sum(record.net_salary for record in payroll_records)
        num_employees = len(payroll_records)
        
        # Estimate ClickPesa fees per bank payout (typically around 0.5-2% or fixed fee)
        # Using 1% as estimate, but this should be adjusted based on actual ClickPesa fee structure
        estimated_clickpesa_fee_per_payout = total_net_salary / num_employees * 0.01 if num_employees > 0 else 0
        total_clickpesa_fees = estimated_clickpesa_fee_per_payout * num_employees
        
        # 1% platform fee per payout
        platform_fee_per_payout = total_net_salary / num_employees * 0.01 if num_employees > 0 else 0
        total_platform_fees = platform_fee_per_payout * num_employees
        
        # Calculate base total (net salary + ClickPesa fees + platform fees)
        base_total = total_net_salary + total_clickpesa_fees + total_platform_fees
        
        # 1% settlement fees on base total
        total_settlement_fees = base_total * 0.01
        
        # Final total amount
        total_amount = base_total + total_settlement_fees
        
        # Generate unique reference
        payment_reference = f"PAYROLL{payroll_period.replace('-', '')}{uuid.uuid4().hex[:8].upper()}"
        
        # Generate BillPay control number
        billpay_control_number = None
        try:
            token = get_clickpesa_token()
            
            billpay_request = {
                "customerName": f"Payroll Payment {payroll_period}",
                "billDescription": f"Payroll payment for {payroll_period} - {num_employees} employees",
                "billPaymentMode": "EXACT_AMOUNT_ONLY",  # Fixed amount
                "billAmount": float(total_amount),
            ...
            billpay_control_number = (
                ...
                billpay_response.get('data', {}).get('controlNumber')
            )
            
            if not billpay_control_number:
                raise Exception("ClickPesa API did not return a control number")
                
        except Exception as e:
            logger.error(f"Error generating BillPay control number: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Failed to generate BillPay control number: {str(e)}")
        
        # Create or update payroll payment record
        if existing_payment:
            existing_payment.total_net_salary = total_net_salary
            existing_payment.total_clickpesa_fees = total_clickpesa_fees
            existing_payment.total_platform_fees = total_platform_fees
            existing_payment.total_settlement_fees = total_settlement_fees
            existing_payment.total_amount = total_amount
            existing_payment.billpay_control_number = billpay_control_number
            existing_payment.billpay_reference = payment_reference
            existing_payment.status = "pending"
            existing_payment.clickpesa_response = json.dumps(billpay_response)
            payroll_payment = existing_payment
        else:
            payroll_payment = PayrollPayment(
                ...
                billpay_control_number=billpay_control_number,
                billpay_reference=payment_reference,
                status="pending",
                clickpesa_response=json.dumps(billpay_response)
            )
        ...
        return {
            ...
            "message": f"Pay {total_amount:.2f} TZS using control number {billpay_control_number} and merchant number {SHARED_BILLPAY_NAMBA}"
        }
...
def process_payroll_payment(payroll_payment_id: int, db: Session):
    """
    Process payroll payment by distributing to employees via ClickPesa bank payouts.
    Called when the BillPay control number is paid.
    """
    try:
        payroll_payment = db.query(PayrollPayment).filter(PayrollPayment.id == payroll_payment_id).first()
        if not payroll_payment:
            raise Exception(f"Payroll payment {payroll_payment_id} not found")
        
        if payroll_payment.status == "completed":
            logger.info(f"Payroll payment {payroll_payment_id} already completed")
            return {"status": "already_completed"}
        
        # Get all payroll records for this period
        query = db.query(PayrollRecord).filter(PayrollRecord.payroll_period == payroll_payment.payroll_period)
        if payroll_payment.branch_id:
            query = query.join(Staff, PayrollRecord.staff_id == Staff.id).filter(Staff.branch_id == payroll_payment.branch_id)
        
        payroll_records = query.join(Staff, PayrollRecord.staff_id == Staff.id).all()
        
        if not payroll_records:
            raise Exception(f"No payroll records found for period {payroll_payment.payroll_period}")
        ...
        # Get ClickPesa service and banks list
        clickpesa_service = ClickPesaService()
        banks_list = clickpesa_service.get_banks_list()
        
        # Create a mapping of bank names to BIC codes
        bank_bic_map = {}
        if isinstance(banks_list, list):
            for bank in banks_list:
                bank_name = bank.get('name', '').upper()
                bic = bank.get('bic') or bank.get('code')
                if bic:
                    bank_bic_map[bank_name] = bic
        
        # Process each employee payout
        ...
        for record in payroll_records:
            ...
            if not staff.bank_account or not staff.bank_name:
                failed_payouts += 1
                payout_results.append({
                    "staff_id": record.staff_id,
                    "name": f"{staff.first_name} {staff.last_name}",
            ...
            # Find bank BIC
            bank_name_upper = staff.bank_name.upper()
            bank_bic = bank_bic_map.get(bank_name_upper)
            
            if not bank_bic:
                # Try partial match
                for bank_key, bic in bank_bic_map.items():
                    if bank_name_upper in bank_key or bank_key in bank_name_upper:
                        bank_bic = bic
                        break
            ...
            if not bank_bic:
                ...
                payout_results.append({
                    ...
                    "name": f"{staff.first_name} {staff.last_name}",
                    "status": "failed",
                    "error": f"Bank '{staff.bank_name}' not found in ClickPesa supported banks"
                })
                continue
            
            # Create bank payout via ClickPesa
            try:
                payout_reference = f"PAYROLL{payroll_payment.payroll_period.replace('-', '')}EMP{record.staff_id}"
                
                payout = clickpesa_service.create_bank_payout(
                    amount=record.net_salary,
                    account_number=staff.bank_account,
                    account_name=staff.account_name or f"{staff.first_name} {staff.last_name}",
                    currency="TZS",
                ...
                payout_results.append({
                    ...
                    "status": "success",
                    "amount": record.net_salary,
                    "clickpesa_reference": payout.get('id') or payout.get('orderReference')
                })
                ...
                logger.error(f"Failed to process payout for staff {record.staff_id}: {str(payout_error)}")
        
        # Update payroll payment status
        if failed_payouts == 0:
            payroll_payment.status = "completed"
        elif successful_payouts > 0:
            payroll_payment.status = "partially_completed"
        else:
            payroll_payment.status = "failed"
        ...
        db.rollback()
        logger.error(f"Error processing payroll payment {payroll_payment_id}: {str(e)}")
        raise

@router.post("/payroll/generate-individual-payment")
async def generate_individual_payroll_payment(
    payroll_record_id: int,
    db: Session = Depends(get_db)
):
    """
    Generate BillPay control number for individual employee payroll payment.
    Calculates total amount including:
    - Employee net salary
    - ClickPesa fee (estimated 1%)
    - 1% platform fee
    - 1% settlement fees
    """
    try:
        ...
        if payroll_record.status == "paid":
            raise HTTPException(status_code=400, detail="This payroll record is already paid")
        
        # Calculate fees
        net_salary = payroll_record.net_salary
        
        # ClickPesa fee (estimated 1%)
        clickpesa_fee = net_salary * 0.01
        
        # Platform fee (1%)
        platform_fee = net_salary * 0.01
        
        # Base total
        base_total = net_salary + clickpesa_fee + platform_fee
        
        # Settlement fee (1% on base total)
        settlement_fee = base_total * 0.01
        
        # Final total
        total_amount = base_total + settlement_fee
        
        # Generate unique reference
        payment_reference = f"PAYROLLIND{payroll_record_id}{uuid.uuid4().hex[:8].upper()}"
        ...
        try:
            ...
            billpay_request = {
                "customerName": f"{staff.first_name} {staff.last_name} - Payroll",
                "billDescription": f"Payroll payment for {staff.first_name} {staff.last_name} - {payroll_record.payroll_period}",
                "billPaymentMode": "EXACT_AMOUNT_ONLY",
                "billAmount": float(total_amount),
                "billReference": payment_reference
            }
        ...
        # Store payment info - create PayrollPayment record for tracking
        individual_payment = PayrollPayment(
            payroll_period=payroll_record.payroll_period,
            branch_id=staff.branch_id,
            payroll_record_id=payroll_record_id,  # Link to specific payroll record
            total_net_salary=net_salary,
            total_clickpesa_fees=clickpesa_fee,
        ...
        db.add(individual_payment)
        db.commit()
        db.refresh(individual_payment)
        ...
        return {
            ...
            "billpay_namba": SHARED_BILLPAY_NAMBA,
            "payment_reference": payment_reference,
            "payment_id": individual_payment.id,
            "message": f"Pay {total_amount:.2f} TZS using control number {billpay_control_number} and merchant number {SHARED_BILLPAY_NAMBA}"
        }
...
def process_individual_payroll_payment(payment_id: int, db: Session):
    """
    Process individual employee payroll payment by sending payout to employee's bank account.
    Called when the BillPay control number is paid.
    """
    try:
        payroll_payment = db.query(PayrollPayment).filter(PayrollPayment.id == payment_id).first()
        if not payroll_payment:
            raise Exception(f"Payroll payment {payment_id} not found")
        
        if payroll_payment.status == "completed":
            logger.info(f"Individual payroll payment {payment_id} already completed")
            return {"status": "already_completed"}
        
        # Get the payroll record - for individual payments, we have payroll_record_id
        if payroll_payment.payroll_record_id:
            payroll_record = db.query(PayrollRecord).filter(PayrollRecord.id == payroll_payment.payroll_record_id).first()
        else:
            # For bulk payments, get first unpaid record for this period
            query = db.query(PayrollRecord).filter(
                PayrollRecord.payroll_period == payroll_payment.payroll_period,
        ...
        if not payroll_record:
            raise Exception(f"No payroll record found for payment {payment_id}")
        ...
        # Check if employee has bank details
        if not staff.bank_account or not staff.bank_name:
            raise Exception(f"Employee {staff.first_name} {staff.last_name} is missing bank account details")
        ...
        if not bank_bic:
            raise Exception(f"Bank '{staff.bank_name}' not found in ClickPesa supported banks")
        
        # Create bank payout via ClickPesa
        payout_reference = f"PAYROLLIND{payroll_record.id}EMP{staff.id}"
        ...
        db.rollback()
        logger.error(f"Error processing individual payroll payment {payment_id}: {str(e)}")
        raise
